...
local netrw_nvim_keys = {
  {
    ...
    function()
      ...
      local netrw_buffers = vim.tbl_filter(function(buf)
  ...
  mappings = {
    ...
    ["n"] = function(payload)
      ...
      local file_name = vim.fn.input({
        prompt = "Enter new file name (end with '/' for directories): ",
        default = "",
        completion = "file",
      })
    ...
    ["D"] = function(payload)
      if payload.type == 0 then
        ...
        local delete_dir = vim.fn.input({
          prompt = "Delete directory " .. payload.node .. " recursively? (y/n): ",
          default = "n",
        })
        ...
        local delete_file = vim.fn.input({
          prompt = "Delete file " .. payload.node .. "? (y/n): ",
          default = "n",
        })
...
fignvim.mappings.create_keymaps(netrw_nvim_keys)
